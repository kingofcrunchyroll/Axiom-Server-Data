...
public class VRRig : MonoBehaviour, IWrappedSerializable, INetworkStruct, IPreDisable, IUserCosmeticsCallback, IGorillaSliceableSimple, ITickSystemPost, IEyeScannable
{
    ...
    public void SliceUpdate()
    {
        ...
        {
            _nextUpdateTime = time + 1f;
            if (RoomSystem.JoinedRoom && NetworkSystem.Instance.IsMasterClient && ((UnityEngine.Object)(object)GameMode.ActiveNetworkHandler).IsNull())
            {
    ...
    public void PostTick()
    {
        ...
        if (isOfflineVRRig)
        {
            ...
            {
                SpeakingLoudness = 0f;
                if (shouldSendSpeakingLoudness && (bool)(UnityEngine.Object)(object)netView)
                {
                    PhotonVoiceView component = ((Component)(object)netView).GetComponent<PhotonVoiceView>();
    ...
    private InputStruct SerializeWriteShared()
    {
        ...
        result.rotation = BitPackUtils.PackQuaternionForNetwork(base.transform.rotation);
        result.usingNewIK = ShouldUseNewIKMethod(myIk.usingUpdatedIK);
    ...
    private void SerializeReadShared(InputStruct data)
    {
        ...
        ref Quaternion q = ref head.syncRotation;
        Quaternion newVal = BitPackUtils.UnpackQuaternionFromNetwork(data.headRotation);
        q.SetValueSafe(in newVal);
        bool usingUpdatedIK = ShouldUseNewIKMethod(data.usingNewIK);
        myIk.usingUpdatedIK = usingUpdatedIK;
        if (myIk.usingUpdatedIK)
        {
            ref Quaternion targetBodyRot = ref myIk.targetBodyRot;
            newVal = BitPackUtils.UnpackQuaternionFromNetwork(data.bodyRotation);
            targetBodyRot.SetValueSafe(in newVal);
            ref Vector3 leftElbowDirection = ref myIk.leftElbowDirection;
            Vector3 newVal2 = BitPackUtils.UnpackRelativePos16((ushort)data.leftUpperArmRotation, Vector3.zero, 1f);
            leftElbowDirection.SetValueSafe(in newVal2);
            ref Vector3 rightElbowDirection = ref myIk.rightElbowDirection;
            newVal2 = BitPackUtils.UnpackRelativePos16((ushort)data.rightUpperArmRotation, Vector3.zero, 1f);
            rightElbowDirection.SetValueSafe(in newVal2);
        }
        ...
        ref Quaternion q2 = ref syncRotation;
        newVal = BitPackUtils.UnpackQuaternionFromNetwork(data.rotation);
        q2.SetValueSafe(in newVal);
        int packedFields = data.packedFields;
        remoteUseReplacementVoice = (packedFields & 0x200) != 0;
        if (((uint)packedFields & 0x400000u) != 0 && SubscriptionManager.GetSubscriptionDetails(this).active)
        {
            ((Graphic)(object)playerText1).color = SubscriptionManager.SUBSCRIBER_NAME_COLOR;
        }
        else
        {
            ((Graphic)(object)playerText1).color = Color.white;
        }
        ...
        if (((uint)packedFields & 0x10000u) != 0)
        {
    ...
    void IWrappedSerializable.OnSerializeWrite(PhotonStream stream, PhotonMessageInfo info)
    {
        ...
        if (((uint)inputStruct.packedFields & 0x10000u) != 0)
        {
    ...
    void IWrappedSerializable.OnSerializeRead(PhotonStream stream, PhotonMessageInfo info)
    {
        ...
        data.rotation = (int)stream.ReceiveNext();
        data.usingNewIK = (bool)stream.ReceiveNext();
        if (data.usingNewIK)
        {
        ...
        if (((uint)data.packedFields & 0x10000u) != 0)
        {
    ...
    public void InitializeNoobMaterial(float red, float green, float blue, PhotonMessageInfoWrapped info)
    {
        ...
        if (info.senderID == NetworkSystem.Instance.GetOwningPlayerID(((Component)(object)rigSerializer).gameObject) && (!initialized || (initialized && GorillaComputer.instance.friendJoinCollider.playerIDsCurrentlyTouching.Contains(userID)) || (initialized && CosmeticWardrobeProximityDetector.IsUserNearWardrobe(info.senderID))))
        {
            initialized = true;
    ...
    public void SetNameTagText(string name)
    {
        ...
        ((TMP_Text)playerText1).text = name;
    ...
    public void UpdateName()
    {
        ...
        bool isNamePermissionEnabled = (permissionDataByFeature.Enabled || (int)permissionDataByFeature.ManagedBy == 1) && (int)permissionDataByFeature.ManagedBy != 3;
        UpdateName(isNamePermissionEnabled);
    }
    ...
    internal void SetHandEffectData(HandEffectContext effectContext, int audioClipIndex, bool isDownTap, bool isLeftHand, StiltID stiltID, float handTapVolume, float handTapSpeed, Vector3 dirFromHitToHand)
    {
        ...
        HashWrapper hash = (handTapOverrides.overrideSurfacePrefab ? handTapOverrides.surfaceTapPrefab : GTPlayer.Instance.materialDatasSO.surfaceEffects[handSurfaceData.surfaceEffectIndex]);
        prefabHashes[0] = hash;
        effectContext.prefabHashes[1] = (handTapOverrides.overrideGamemodePrefab ? ((int)handTapOverrides.gamemodeTapPrefab) : ((RoomSystem.JoinedRoom && GameMode.ActiveGameMode.IsNotNull()) ? GameMode.ActiveGameMode.SpecialHandFX(creator, rigContainer) : (-1)));
    ...
    public void PlaySplashEffect(Vector3 splashPosition, Quaternion splashRotation, float splashScale, float boundingRadius, bool bigSplash, bool enteringWater, PhotonMessageInfoWrapped info)
    {
        ...
        {
            float maxVal = 10000f;
            if (splashPosition.IsValid(in maxVal) && splashRotation.IsValid() && float.IsFinite(splashScale) && float.IsFinite(boundingRadius))
            {
    ...
    public void UpdateCosmeticsWithCollectables(int[] cycleStatesPacked, PhotonMessageInfoWrapped info)
    {
        ...
        for (int i = 0; i < num; i++)
        {
            ...
            if (num3 >= 0)
            {
                ...
                CosmeticsController.CollectionState collectionState = default(CosmeticsController.CollectionState);
                collectionState.activeIndex = num3;
                collectionState.visibleMask = visibleMask;
                CosmeticsController.CollectionState value = collectionState;
    ...
    public void SetCollectionCycleIndex(int packedParentID, int activeIndex, int visibleMask, PhotonMessageInfoWrapped info)
    {
        ...
        {
            CosmeticsController.CollectionState collectionState = default(CosmeticsController.CollectionState);
            collectionState.activeIndex = activeIndex;
            collectionState.visibleMask = visibleMask;
            CosmeticsController.CollectionState value = collectionState;
            remoteCycleStates[text] = value;
            CosmeticCollectionDisplay cosmeticCollectionDisplay = CosmeticCollectionDisplay.FindForRig(this, text);
    ...
    {
        if ((UnityEngine.Object)(object)netView != null && _scoreUpdated)
        {
    ...
    {
        if ((UnityEngine.Object)(object)netView != null && _rankedInfoUpdated && newGameModeType != GameModeType.InfectionCompetitive && !m_sentRankedScore)
        {
    ...
    {
        if ((UnityEngine.Object)(object)netView != null)
        {
            if (_scoreUpdated)
            {
    ...
    public void SetRankedInfo(float rankedELO, int rankedSubtierQuest, int rankedSubtierPC, bool broadcastToOtherClients = true)
    {
        ...
        if ((UnityEngine.Object)(object)netView != null && broadcastToOtherClients)
        {
            netView.SendRPC("RPC_UpdateRankedInfo", RpcTarget.Others, currentRankedELO, currentRankedSubTierQuest, currentRankedSubTierPC);
        }
    }
    ...
    private bool IsInRankedMode()
    {
        ...
        {
            return GameMode.ActiveGameMode.GameType() == GameModeType.InfectionCompetitive;
        }
    ...
    public void OnEnable()
    {
        ...
        if (isOfflineVRRig)
        {
            ...
            GameMode.OnStartGameMode += SendScoresToGameModeRoom;
    ...
    public void NetInitialize()
    {
        ...
        if ((UnityEngine.Object)(object)netView != null)
        {
            base.transform.position = ((Component)(object)netView).gameObject.transform.position;
            base.transform.rotation = ((Component)(object)netView).gameObject.transform.rotation;
        }
    ...
    {
        if (!(UnityEngine.Object)(object)netView || netView.IsMine)
        {
            return false;
        }
    ...
    private void OnKIDSessionUpdated(bool showCustomNames, ManagedByEnum managedBy)
    {
        ...
        bool flag = (showCustomNames || (int)managedBy == 1) && (int)managedBy != 3;
